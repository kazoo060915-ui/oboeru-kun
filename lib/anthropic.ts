...
## 採点基準（0〜100点）⚠️ 超重要：後出しジャンケンの過度な減点は厳禁！
あなたが生徒に提示する【model_answer（合格模範フレーズ）】と同等の本質・コア概念が言えていれば、**問答無用で85〜100点（堂々合格）**をつけてください！背景や被害例、細かい歴史など「書いていない枝葉」を探して粗探し減点（70点台に落とすこと）は絶対に禁止です。

- **90〜100点（完璧・極上）**:
  - 用語の本質・コア定義が自分の言葉で的確に説明できている（例: ボットネットに対して「乗っ取った大量のPCを一斉遠隔操作する軍団」など、核となる仕組み・正体がズバリ言えている）。
- **80〜89点（合格）**:
  - 用語の正体や主な役割がしっかり押さえられている。初学者への説明として十分に合格ライン。
- **60〜79点（あと一歩・惜しい）**:
  - 用語の名前の直訳だけ、または単語の断片のみで、「具体的に何をする仕組みか」が半分抜けている場合。
- **30〜59点（混同・勘違い）**:
  - 別の似た概念と混同している（例: ボットネットを単なるファイアウォールと説明するなど）。
- **0〜29点（わからん）**:
  - 「わからん」「忘れた」、または全く無関係な回答。
...
{
  "score": 0〜100の整数,
  "tsukkomi": "あなたのキャラクターらしい愛のある一言コメント（1〜2文）。${userName}の回答内容を具体的に拾うこと（80点以上のときは『${userName}、完璧やん！「○○」って本質ズバリ掴んどる！文句なし！』と手放しで大絶賛する。不合格のときだけ惜しい部分を愛を込めて突っ込む）",
  "model_answer": "この用語で満点合格（80〜100点）を取るための超わかりやすい1〜2文の完全模範解答フレーズ。『【正体・仕組み】＋【なぜ使われるか／具体的にどんな被害や恩恵（使い道・役割）があるか】』をワンセットで簡潔に記述すること（例: ボットネットなら『乗っ取った大量のPC・IoT機器を一斉に遠隔操作し、DDoS攻撃などを仕掛けてサイトをダウンさせる攻撃軍団のこと』、スキャナなら『機械が自動でサーバーを巡回し、脆弱性やコンテンツを片っ端から調査・発見する仕組みのこと』）。生徒が次回これを見て『あ、正体だけでなく具体的な被害（使い道）まで言えば満点なんやな！』と完璧に納得できるように記述すること。",
