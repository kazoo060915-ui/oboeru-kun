...

{
  ...
  "mission": "今すぐ10秒〜1分でその場で体感・実行できる超具体的なミニ課題を1つ。生徒は【スマホ（LINE経由）で復習している可能性が高い】ため、ターミナルやPCのコマンド（curl等）を必須にするのは絶対に禁止！『スマホのブラウザで実際に○○のページを開いて挙動やURLを確かめる』『身近な有名サイト（AmazonやYouTube等）を開いて○○の箇所をタップしてみる』『一生忘れない語呂合わせを声に出して1回呟いてみる』『下のチャットでコーチに「○○についてもっと教えて」と質問してみる』など、スマホでも手元で今すぐ100%できる超実践アクションを指定すること（PC向けに「※PCならターミナルで○○も試せるで」と一言添えるのはOK）。抽象的な「調べてみよう」は禁止"
}`;
}
...
function buildFallbackGradeResult(term: string, body: string, coach: CoachType = 'osaka', userName: string = 'あなた'): GradeResult {
  ...
  if (isWakaran) {
    ...
    return {
      ...
      correct: correctText,
      missed: [term, '仕組み'],
      mission: `スマホのブラウザで解説の中に出てきた語呂合わせを声に出して1回呟いてみるか、下のチャットで「${term}」の別の例えを聞いてみよう！`,
      related: [],
    };
  }
  ...
  return {
    ...
    correct: correctText,
    missed: [term, '本質', 'メリット'],
    mission: `解説の中に出てきた語呂合わせやポイントを声に出して1回言ってみるか、下のチャットでコーチに「実務ではどう使う？」と聞いてみよう！`,
    related: [],
  };
}
